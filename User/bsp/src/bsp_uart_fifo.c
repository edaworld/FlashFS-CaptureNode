...
void UART_InitALL(void)
{
	...
	UART_NVIC_Configuration(); /* 配置串口中断，设置各个串口的优先级等级和中断服务函数*/
}
...
void COMx_SendBuf(COM_PORT_E _ucPort, uint8_t *_ucaBuf, uint16_t _usLen)
{
	...
	UartSend(pUart, _ucaBuf, _usLen);//发送数据
}
...
static void UART_NVIC_Configuration(void)
{
	...
	/*  NVIC_PriorityGroupConfig(NVIC_PriorityGroup_1);  --- 在 bsp.c 中 bsp_Init() 中配置中断优先级组 */
